    ...
    try:
        ...
        if not db.is_admin(message.from_user.id):
            await message.reply_text(
                f"⚠️ You are not authorized to use this command.\n\n{CREDIT}",
                parse_mode="HTML",
        ...
        try:
            ...
            await message.reply_text(
                f"❌ Invalid channel ID. Please use a valid number.\n\n{CREDIT}",
                parse_mode="HTML",
                disable_web_page_preview=True
            )
            return
        ...
        if db.set_log_channel(client.me.username, channel_id):
            ...
            await message.reply_text(
                f"❌ Failed to set log channel. Please try again.\n\n{CREDIT}",
                parse_mode="HTML",
                disable_web_page_preview=True
            )
    ...
    try:
        ...
        if channel_id:
            ...
            await message.reply_text(
                ...
                f"{channel_info}"
                f"🆔 Channel ID: `{channel_id}`\n\n"
                f"Use /setlog to change the log channel\n\n{CREDIT}",
                parse_mode="HTML",
                disable_web_page_preview=True
            )
        else:
            await message.reply_text(
                f"**📋 Log Channel Info**\n\n"
                f"🤖 Bot: @{client.me.username}\n"
                f"❌ No log channel set\n\n"
                f"Use /setlog to set a log channel\n\n{CREDIT}",
                parse_mode="HTML",
                disable_web_page_preview=True
            )
...
photologo = 'https://imagefa.st/images/2025/12/04/WhatsApp-Image-2025-12-04-at-1.26.58-PM.th.jpeg'
photoyt = 'https://imagefa.st/images/2025/12/04/WhatsApp-Image-2025-12-04-at-1.26.58-PM.th.jpeg'
...
async def cookies_handler(client: Client, m: Message):
    await m.reply_text(
        f"Please upload the cookies file (.txt format).\n\n{CREDIT}",
        quote=True,
        parse_mode="HTML",
        disable_web_page_preview=True
    )
    ...
    try:
        ...
        if not input_message.document or not input_message.document.file_name.endswith(".txt"):
            await m.reply_text(
                f"Invalid file type. Please upload a .txt file.\n\n{CREDIT}",
                parse_mode="HTML",
                disable_web_page_preview=True
            )
            return
...
async def text_to_txt(client, message: Message):
    ...
    await editable.edit(
        f"**🔄 Send file name or send /d for filename**\n\n{CREDIT}",
        parse_mode="HTML",
        disable_web_page_preview=True
    )
...
async def getcookies_handler(client: Client, m: Message):
    try:
        await client.send_document(
            ...
            caption=f"Here is the `youtube_cookies.txt` file.\n\n{CREDIT}",
            parse_mode="HTML"
        )
    except Exception as e:
        await m.reply_text(
    ...
    try:
        if m.chat.type == "channel":
            ...
            await m.reply_text(
                ...
                f"Send these commands in the channel to use them.\n\n{CREDIT}",
                parse_mode="HTML",
                disable_web_page_preview=True
            )
        else:
            ...
            if not is_authorized:
                await m.reply_photo(
                    photo=photologo,
                    caption=(
                        "**Mʏ Nᴀᴍᴇ [DRM Wɪᴢᴀʀᴅ 🦋](https://t.me/stain1260bot)\n\n"
